...
export const useLogisticsStore = create<LogisticsState>((set, get) => ({
    ...
    fetchData: async () => {
        ...
        try {
            const response = await fetch('/api/pallets', { credentials: 'include' });
    ...
    applyPalletAction: (palletId, action, overrides) => {
        ...
        // Persist via the Next.js API route in the background — UI stays optimistic
        void fetch(`/api/pallets/${palletId}/actions`, {
            method: 'POST',
            credentials: 'include',
