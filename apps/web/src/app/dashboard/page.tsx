...
export default function DashboardPage() {
  ...
  const eventsByPallet = useMemo(
    () => groupEventsByPallet(palletEvents),
    [palletEvents]
  );
  const selectedPalletEvents = selectedPallet
    ? (eventsByPallet.get(selectedPallet.id) ?? [])
    : [];
  const kpis = useMemo(
    () => calculateLogisticsKpis(pallets, palletEvents),
    [pallets, palletEvents]
  );
    ...
    <SidebarInset className="relative h-dvh bg-[#F4F4F5] overflow-hidden font-sans flex flex-col">
      ...
      <div className="relative z-10 flex flex-col h-full overflow-hidden pointer-events-none">
        ...
        <div
          ...
          style={{
            top: isListExpanded ? "4rem" : `${100 - BASE_LIST_PANEL_HEIGHT_DVH}dvh`,
          }}
        >
          <MagicSearchbar />
        </div>
        ...
        <div
          ...
          style={{
            height: isListExpanded ? "calc(100dvh - 4rem)" : `${BASE_LIST_PANEL_HEIGHT_DVH}dvh`,
          }}
        >
...
function HeaderKPIChip({
  ...
  return (
    ...
    >
      <Icon
        className={cn("size-3.5", alert ? "text-red-600" : "text-[#BC804C]")}
      />
